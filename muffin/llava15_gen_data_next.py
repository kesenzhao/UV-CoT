...
def wrap_question_for_llava15(item, tokenizer, mm_use_im_start_end, conv_mode):
    ...
    conv.append_message(conv.roles[1], item['answers'])
    conv.append_message(conv.roles[0], DEFAULT_IMAGE_TOKEN + '\nPlease answer the question based on the original image and local detail image.'+  qs.split('Please provide the bounding box coordinate of the region')[0].replace('<image>\n', ''))
...
def llava15_qa_colloator_fn(data_list, tokenizer, image_processor, config):
    ...
    for x in data_list:
        ...
        images = [image]
        try:
            coords = re.findall(r'[-+]?\d*\.\d+', x['metainfos']['answers'])  
            coords = [float(x) for x in coords]
        except:
            print("Can not parse the coords: %s" % x['metainfos']['answers'])
