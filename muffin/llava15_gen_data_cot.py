...
# def save_outputs(outputs, output_file, rank, batch_idx):
#     timestamp = datetime.now().strftime("%Y%m%d%H%M%S")
#     with open(f"{output_file}_rank{rank}_batch{batch_idx}_{timestamp}.jsonl", 'a', encoding='utf-8') as f:
#         for output in outputs:
#             f.write(json.dumps(output) + '\n')

# def merge_files(output_file, world_size):
#     with open(output_file, 'w', encoding='utf-8') as outfile:
#         for rank in range(world_size):
#             rank_files = [f for f in os.listdir() if f.startswith(f"{output_file}_rank{rank}_batch")]
...
class GenDataset(torch_data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if index%10 == 0:
            ...
            center = [random.randint(0, 12), random.randint(0, 12)]
            H = 24
            W = 24
            x1, y1 = round(center[0] / H, 2), round(center[1] / H, 2)
            x2, y2 = round(x1+0.5, 2), round(y1+0.5, 2)
            coords = [x1, y1, x2, y2]
        metainfo['bbox'] = coords
...
def llava15_qa_colloator_fn(data_list, tokenizer, image_processor, config):
    ...
    for x in data_list:
        images = []
        coords = x['bbox']
        bbox_batch.append(coords)
        image = x['image']
        images.append(image)
        image = cropwithbbox(image, coords)
        images.append(image)
        images_batch.append(process_images(images, image_processor, config)[0])
    ...
    with torch.inference_mode():
        for batch in tqdm.tqdm(dataloader, f'Generating answers'):
            ...
            if args.is_yesno:
                ...
                for question, output_ids, question_id, metainfos, bbox in zip(batch['raw_questions'], output.sequences, batch['question_id'], batch['metainfos'], batch['bbox']):
                    response = tokenizer.decode(
                            output_ids, skip_special_tokens=True)
                    response = response.strip()
