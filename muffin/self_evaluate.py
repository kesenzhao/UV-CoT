...
class GenDataset(torch_data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        question_input_ids = self.question_process(item)
...
def wrap_question_for_llava15(item, tokenizer, mm_use_im_start_end, conv_mode):
    qs = item['question'] 
    ...
    conv.append_message(conv.roles[0], qs.split('Please answer yes or no.')[0] + 'Please provide the bounding box coordinate of the region')
    conv.append_message(conv.roles[1], item['metainfos']['answers'])
    conv.append_message(conv.roles[0], DEFAULT_IMAGE_TOKEN + '\nPlease answer the question based on the original image and local detail image.'+  qs.replace('<image>\n', ''))
...
def llava15_qa_colloator_fn(data_list, tokenizer, image_processor, config):
    ...
    for x in data_list:
        ...
        images = [image]
        try:
            coords = re.findall(r'[-+]?\d*\.\d+', x['metainfos']['metainfos']['answers'])  
            coords = [float(x) for x in coords]
        except:
            print("Can not parse the coords: %s" % x['metainfos']['metainfos']['answers'])
        ...
        image_emb=process_images(images, image_processor, config)
        images_emb.append(image_emb)
    images_emb = torch.stack(images_emb, dim=0).reshape(-1,3,image_emb.shape[-1],image_emb.shape[-1])
