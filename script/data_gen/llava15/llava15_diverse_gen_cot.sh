...
# torchrun --nnodes=1 --nproc_per_node=${num_gpus} --rdzv_id=1 --rdzv_backend=c10d --rdzv_endpoint=localhost:0 ./muffin/llava15_gen_data_bbox.py \
# --checkpoint $ckpt \
...
torchrun --nnodes=1 --nproc_per_node=${num_gpus} --rdzv_id=1 --rdzv_backend=c10d --rdzv_endpoint=localhost:0 ./muffin/llava15_gen_data_cot.py \
--checkpoint $ckpt \
--ds_name ${ques_dir}/${ques_file} \
--answer_file ${ans_dir}/diverse_gen_llava15_${start_pos}-${end_pos}_${ques_file} \
--max_sample -1 \
