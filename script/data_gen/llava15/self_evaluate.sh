...
torchrun --nnodes=1 --nproc_per_node=${num_gpus} --rdzv_id=1 --rdzv_backend=c10d --rdzv_endpoint=localhost:0 ./muffin/self_evaluate.py \
--checkpoint $ckpt \
--ds_name ${ques_dir}/${ques_file} \
--answer_file  ${ans_dir}/autocheck_llava_${start_pos}-${end_pos}_${ques_file} \
--max_sample -1 \
--start_pos $start_pos \
--end_pos $end_pos \
--max_tokens 1 \
