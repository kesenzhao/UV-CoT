...
def zephyr_qa_colloator_fn(data_list, tokenizer, img_transform):
    ...
    for x in data_list:
        image = x['image']
        try:
            coords = re.findall(r'[-+]?\d*\.\d+', x['metainfos']['origin_answer'])  
            coords = [float(x) for x in coords]
        except:
            print("Can not parse the coords: %s" % x['metainfos']['origin_answer'])
            coords = [0.25, 0.25, 0.75, 0.75]
        crop_image= cropwithbbox(x['image'], )
        image=img_transform(image)
        crop_image=img_transform(image)
        images_emb=torch.stack([image, crop_image], dim=0)
        images.append(images_emb)
